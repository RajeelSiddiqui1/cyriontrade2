...
api.interceptors.request.use(
  async (config) => {
    const token = localStorage.getItem('token'); // Changed to match my context or I'll update context
    if (token) {
...
export const authAPI = {
  ...
  purchase: (userId, itemId, price, category) => api.post('/auth/purchase', { userId, itemId, price, category }),
};
...
export const cryptoAPI = {
  ...
  getOHLCData: (id, days = 1, currency = 'USD') =>
    api.get(`/crypto/ohlc/${id}`, { params: { days, currency } }),
